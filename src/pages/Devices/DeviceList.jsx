...
import { Plus, Edit, Trash2, Wifi, Power, RefreshCw, Activity, Monitor, Eye } from 'lucide-react'
...
const DeviceList = () => {
  ...
  const fetchDevices = async () => {
    try {
      ...
      const devicesArray = Array.isArray(response.rpis) ? response.rpis : 
  ...
  const handleUpdateDevice = async (data) => {
    try {
      const response = await deviceService.updateDevice(editingDevice._id, data)
      if (response.rpis) {
  ...
  const handleBulkStatusUpdate = async (status) => {
    ...
    try {
      ...
      toast.success(`${selectedDevices.length} devices ${status === 'active' ? 'activated' : 'deactivated'}`)
      setSelectedDevices([])
    } catch (error) {
      toast.error('Failed to update device status')
      console.error('Error updating device status:', error)
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedDevices.length} devices? This action cannot be undone.`)) return
  ...
  const getStatusConfig = (status) => {
    const statusConfig = {
      active: { color: 'green', label: 'Active', bgColor: 'bg-green-100', textColor: 'text-green-800' },
      in_active: { color: 'red', label: 'Inactive', bgColor: 'bg-red-100', textColor: 'text-red-800' },
      warning: { color: 'yellow', label: 'Warning', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' },
      maintenance: { color: 'blue', label: 'Maintenance', bgColor: 'bg-blue-100', textColor: 'text-blue-800' }
    }
  ...
  const sortedDevices = Array.isArray(devices) ? [...devices].sort((a, b) => {
    ...
    // Handle date sorting
    if (sortBy === 'last_seen' || sortBy === 'createdAt' || sortBy === 'updatedAt') {
    ...
    if (sortBy === 'rpi_status') {
      const statusOrder = { 'active': 1, 'warning': 2, 'maintenance': 3, 'in_active': 4 }
  ...
  const columns = [
    {
      ...
      header: (
        <input
          type="checkbox"
          checked={Array.isArray(devices) && selectedDevices.length === devices.length && devices.length > 0}
          onChange={(e) => {
    ...
    },
    {
      ...
      render: (device) => (
        <div>
          <div className="text-sm text-gray-900">{device.location || 'Not specified'}</div>
          {device.owner_name && (
        ...
        return (
          <div className="flex items-center space-x-2">
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${config.bgColor} ${config.textColor}`}>
              <div className={`w-2 h-2 rounded-full bg-${config.color}-500 mr-1`}></div>
              {config.label}
            </span>
            {device.last_seen && (
    ...
    },
    {
      ...
      sortable: true,
      render: (device) => {
        if (!device.last_seen) return <span className="text-gray-400">Never</span>
  ...
  const activeDevicesCount = devicesArray.filter(d => d.rpi_status === 'active').length
  const inactiveDevicesCount = devicesArray.filter(d => d.rpi_status === 'in_active').length
  const warningDevicesCount = devicesArray.filter(d => d.rpi_status === 'warning').length
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Manage your Raspberry Pi advertising displays ({devicesArray.length} total devices)
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">{activeDevicesCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">{inactiveDevicesCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">{warningDevicesCount}</p>
            </div>
          </div>
        </div>
      </div>
